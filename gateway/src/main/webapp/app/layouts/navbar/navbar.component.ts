...
export class NavbarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.accountService.getAuthenticationState().subscribe(account => {
      ...
      import('conti/entity-navbar-items').then(
        async ({ EntityNavbarItems: ContiEntityNavbarItems }) => {
          this.contiEntityNavbarItems = ContiEntityNavbarItems;
          const { LazyTranslationModule } = await import('conti/translation-module');
          this.loadModule(LazyTranslationModule as Type<any>);
        },
        error => {
          // eslint-disable-next-line no-console
          console.log('Error loading conti entities', error);
        }
      );
    });
  }
